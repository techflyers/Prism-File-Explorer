...
public class CellReader
{
    ...
    private static ThreadLocal<CellReader> reader = new ThreadLocal<CellReader>() {
        @Override
        protected CellReader initialValue() {
    ...
    public  Cell getCell(Sheet sheet, Element cellElement)
    {
        ...
        else if(cellType == CELLTYPE_INLINESTRING)
        {
            // Inline strings store text in <is> child, not <v>.
            // Format: <is><t>text</t></is>  OR  <is><r><t>text</t></r>...</is> (rich text)
            Element is = cellElement.element("is");
            if(is != null)
            {
                Element t = is.element("t");
                String inlineText;
                if(t != null)
                {
                    // Simple inline string
                    inlineText = t.getText();
                }
                else
                {
                    // Rich-text inline string: concatenate all <r><t> run texts
                    StringBuilder sb = new StringBuilder();
                    for(Object child : is.elements("r"))
                    {
                        Element r = (Element) child;
                        Element rt = r.element("t");
                        if(rt != null)
                        {
                            sb.append(rt.getText());
                        }
                    }
                    inlineText = sb.toString();
                }
                cell.setCellValue(book.addSharedString(inlineText));
            }
        }
