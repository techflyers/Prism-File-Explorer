...
public class SheetReader
{
    ...
    private static ThreadLocal<SheetReader> reader = new ThreadLocal<SheetReader>() {
        @Override
        protected SheetReader initialValue() {
            return new SheetReader();
        }
    };   
    ...
    public  void getSheet(IControl control, ZipPackage zipPackage, Sheet sheet, PackagePart  sheetPart, IReader iReader) throws Exception
    {
        ...
        if(ele.element("pane") != null)
        {
           ...
           {
               paneInfo.setVerticalSplitLeftColumn((short)Math.round(Double.parseDouble(pane.attributeValue("xSplit"))));
           }
           
           if(pane.attributeValue("ySplit") != null)
           {
               paneInfo.setHorizontalSplitTopRow((short)Math.round(Double.parseDouble(pane.attributeValue("ySplit"))));
           }
    ...
    private void setColumnProperty(Element col)
    {
        ...
        {
            String val = col.attributeValue("hidden");
            hidden = "true".equalsIgnoreCase(val) || "1".equals(val);
        }
        else
        {
            hidden = false;
    ...
    class XLSXSaxHandler implements ElementHandler
    {
        ...
        {
            if(iReader != null && iReader.isAborted())
            {                
    ...
    private  Row createRow(Element rowElement, int defaultRowHeight)
    {
        ...
        {
            String val = rowElement.attributeValue("hidden");
            hidden = "true".equalsIgnoreCase(val) || "1".equals(val);          
        }       
