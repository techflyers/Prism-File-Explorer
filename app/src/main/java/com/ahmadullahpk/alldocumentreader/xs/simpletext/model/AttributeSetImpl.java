...
public class AttributeSetImpl implements IAttributeSet
{
    ...
    {   
        ...
        IAttributeSet attrSet = style.getAttrbuteSet();
        if (attrSet == null || !(attrSet instanceof AttributeSetImpl))
        {
            if (style.getBaseID() >= 0)
            {
                Style baseStyle = StyleManage.instance().getStyle(style.getBaseID());
                return getAttributeForStyle(baseStyle, attrID);
            }
            return Integer.MIN_VALUE;
        }
        AttributeSetImpl attr = (AttributeSetImpl)attrSet;
