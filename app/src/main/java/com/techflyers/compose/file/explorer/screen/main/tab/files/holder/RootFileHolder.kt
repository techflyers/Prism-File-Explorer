...
class RootFileHolder : ContentHolder() {
    ...
    override suspend fun listContent(): ArrayList<out ContentHolder> {
        ...
        virtualRootContent.forEach { item ->
            File(item).let { file ->
                if (file.exists()) {
                    ...
                    content.add(com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuFileHolder.fromPath(item))
                }
            }
        }
        ...
        if (com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.isPrivileged) {
            val rootHolder = com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuFileHolder.fromPath("/")
            try {
