...
import com.techflyers.compose.file.explorer.common.hasParent
...
import com.techflyers.compose.file.explorer.screen.viewer.comic.ComicViewerActivity
import com.techflyers.compose.file.explorer.screen.viewer.djvu.DjvuViewerActivity
import com.techflyers.compose.file.explorer.screen.viewer.document.DocumentViewerActivity
import com.techflyers.compose.file.explorer.screen.viewer.epub.EpubViewerActivity
import com.techflyers.compose.file.explorer.screen.viewer.html.HtmlViewerActivity
import com.techflyers.compose.file.explorer.screen.viewer.image.ImageViewerActivity
import com.techflyers.compose.file.explorer.screen.viewer.latex.LatexViewerActivity
import com.techflyers.compose.file.explorer.screen.viewer.markdown.MarkdownViewerActivity
import com.techflyers.compose.file.explorer.screen.viewer.pdf.PdfViewerActivity
...
class LocalFileHolder(file: File) : ContentHolder() {
    ...
    override suspend fun listContent(): ArrayList<out ContentHolder> {
        ...
        if (com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.isPrivileged) {
            val shizukuEntry = com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuFileHolder.fromPath(file.absolutePath)
    ...
    override suspend fun createSubFile(name: String, onCreated: (ContentHolder?) -> Unit) {
        File(file, name).let { newFile ->
            ...
            if (com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.isPrivileged) {
                if (com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.createFile(newFile.absolutePath)) {
                    onCreated(LocalFileHolder(newFile))
                    return
                }
            }
    ...
    override suspend fun createSubFolder(name: String, onCreated: (ContentHolder?) -> Unit) {
        File(file, name).let { newFolder ->
            ...
            if (com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.isPrivileged) {
                if (com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.createDirectory(newFolder.absolutePath)) {
                    onCreated(LocalFileHolder(newFolder))
                    return
                }
            }
    ...
    fun writeText(text: String) {
        try {
            ...
            if (com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.isPrivileged) {
                if (!com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.writeText(file.absolutePath, text)) {
                    throw e
                }
            } else {
                throw e
    ...
    fun readText(): String {
        return try {
            ...
            if (com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.isPrivileged) {
                com.techflyers.compose.file.explorer.screen.main.tab.files.shizuku.ShizukuManager.readText(file.absolutePath) ?: throw e
            } else {
                throw e
            }
        }
