package com.techflyers.compose.file.explorer.saf
...
import com.techflyers.compose.file.explorer.screen.main.tab.files.service.remote.RemoteConnectionPool
import com.techflyers.compose.file.explorer.screen.main.tab.files.service.remote.RemotePaths
import java.io.File
...
class PrismDocumentsProvider : DocumentsProvider() {
    ...
    companion object {
        const val AUTHORITY = "com.techflyers.compose.file.explorer.documents"
        ...
        if (isRemoteDoc(parentDocumentId)) {
            ...
            val items = try {
                client.listDirectory(remotePath).sortedWith(
                    compareBy<com.techflyers.compose.file.explorer.screen.main.tab.files.service.remote.RemoteFileItem> { !it.isDirectory }
    ...
    override fun querySearchDocuments(
        ...
        if (rootId.startsWith("remote_")) {
            ...
            val client = RemoteConnectionPool.clientFor(conn)
            val matches = mutableListOf<com.techflyers.compose.file.explorer.screen.main.tab.files.service.remote.RemoteFileItem>()
    ...
    private fun searchRemoteFiles(
        client: com.techflyers.compose.file.explorer.screen.main.tab.files.service.remote.RemoteClient,
        path: String,
        query: String,
        matches: MutableList<com.techflyers.compose.file.explorer.screen.main.tab.files.service.remote.RemoteFileItem>,
        maxCount: Int,
        maxDepth: Int
    ) {
